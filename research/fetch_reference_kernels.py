"""Restore pinned GPU-MODE reference-kernels files from a clean clone; never rewrite manifests.

Scope: the pmpp_v2 and linalg problem sets only (per-problem closure of
task.yml / task.py / reference.py / submission.py / eval.py / validation.py
plus set-level shared protocol files and the set metadata yaml), plus the
upstream LICENSE. Everything else (amd*, helion, nvidia/Blackwell-fp4, ...)
is out of scope for this project and is not downloaded.

License note (also recorded in configs/reference-kernels/dev-manifest.json):
upstream is distributed under the "June 9 Researcher Reciprocity License
v1.0" (an Open RAIL-S derivative). This project uses the pinned problem set
for local evaluation only: no training on the material, no few-shot or
retrieval reuse in candidate generation, no redistribution of the cached
bytes. All locally produced numbers are replications of the upstream
protocol on hardware the official leaderboard does not list and must never
be compared against official leaderboard results.
"""
...
DEFAULT_ROOT = REPO_ROOT / "research/sources/gpu-mode__reference-kernels"
FILES_MANIFEST = REPO_ROOT / "configs/reference-kernels/snapshot-files.manifest.json"
DEV_MANIFEST = REPO_ROOT / "configs/reference-kernels/dev-manifest.json"

REPOSITORY = "gpu-mode/reference-kernels"
IN_SCOPE_SETS = ("pmpp_v2", "linalg")
TOP_LEVEL_FILES = ("LICENSE",)


def upstream_paths(commit: str) -> list[str]:
    """File-dependency closure for the in-scope problem sets at ``commit``:
    everything under problems/<set>/ plus problems/<set>.yaml metadata."""
    ...
    for item in tree.get("tree", []):
        ...
        if path in TOP_LEVEL_FILES:
            wanted.add(path)
            continue
        for set_name in IN_SCOPE_SETS:
            if path.startswith(f"problems/{set_name}/"):
                wanted.add(path)
            elif path == f"problems/{set_name}.yaml":
                wanted.add(path)
    if not wanted:
        raise ValueError("upstream tree produced no in-scope snapshot candidates")
    for set_name in IN_SCOPE_SETS:
        if f"problems/{set_name}.yaml" not in wanted:
            raise ValueError(f"set metadata missing from upstream tree: {set_name}")
...
def generate_manifest(commit: str, workers: int) -> dict:
    """Download every in-scope file once and record content hashes. The
    output schema matches configs/kernelbench/snapshot-files.manifest.json."""
    if not re.fullmatch(r"[0-9a-f]{40}", commit):
        raise ValueError("commit must be 40-hex")
    paths = upstream_paths(commit)
    ...
    problems = [
        item
        for item in files
        if "/eval.py" not in item["path"] and item["path"].startswith("problems/")
    ]
    return {
        "schema_version": "1.0",
        ...
        "commit": commit,
        "note": (
            "Problem sets pmpp_v2 and linalg only, per-problem file closure "
            "plus set-level eval/utils/template and set metadata yaml. "
            "License: June 9 Researcher Reciprocity v1.0 - local evaluation "
            "only, no training or retrieval reuse, no redistribution."
        ),
        "files": sorted(files, key=lambda item: item["path"]),
        "counts": {"problems_dirs": len(problems), "files": len(files)},
    }
...
def inventory(files_path: Path, dev_path: Path) -> tuple[str, dict[str, str]]:
    ...
    # Note: unlike the KernelBench manifest layout, the rk protocol files
    # (set-level eval.py/utils.py/template.py, per-problem reference/task
    # sources, LICENSE) are already members of the files manifest, so the
    # dev manifest's protocol section carries hashes only and adds no
    # separate download entries here.
...
def main() -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for future in concurrent.futures.as_completed(pending):
            try:
                future.result()
            except Exception as exc:  # noqa: BLE001 - collected per-file, not swallowed
