...
function HealthChip() {
  ...
  return (
    <span className="flex items-center gap-2 text-xs text-muted">
      ...
      <span className="font-mono">{data.gpu_device}</span>
      {data.active_run_id ? (
...
export function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-10 flex h-14 items-center justify-between gap-4 border-b border-border bg-bg/85 px-4 backdrop-blur md:px-6">
      <h2 className="text-sm font-medium text-fg">{sectionLabel(pathname)}</h2>
      <div className="flex items-center gap-3">
        <Badge
          variant="warning"
          className="hidden lg:inline-flex"
          // 诚实性声明：candidate_trust=cooperative 意味着 champion 需人工审查
